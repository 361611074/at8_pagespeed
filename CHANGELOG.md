...
**面向 Z-Blog 应用中心审核的合规迭代。无新业务功能，不新增 Hook，不改系统行为。**

### 新增

- **`THIRD-PARTY-LICENSES.md`**：记录内置第三方组件 instant.page v5.2.0 的版本、来源、许可证（MIT）
  与完整性校验。**源码与上游 `instantpage/instant.page` 仓库 master 分支逐字节一致**
  （17175 B，MD5 `9b05e316aedabe445698f148c00e9e5e`），未作任何修改。
- **「高级 → 清除插件配置」入口**：解决「生命周期钩子不能删配置」带来的清理缺口。
  点击后二次确认，仅删除 `at8_pagespeed` 自己在配置表中的那一行（官方 `DelConfig()`），
  不动系统配置、其它插件与任何文章 / 用户 / 附件数据。
- **`scripts/market-audit.py`**：市场审核自动化检查脚本（版本一致性 / 插件 ID / 危险函数 /
  远程通信 / 动态包含 / 文件操作 / SQL / 日志 / 输出转义 / CSS / JS 命名空间 / 目录污染 /
  PHP 版本语法兼容 / 发布包内容）。
- **`release-manifest.txt` / `release-forbidden.txt`**：发布包内容白名单与黑名单。
- **`docs/`**：Hook 审计、市场审核报告、安全审计报告、生命周期审计报告、
  第三方许可证报告、发布包审计报告、测试矩阵。

### 合规加固

- **CSS class 前缀** `.ps-*` → `.at8ps-*`（原前缀过短，易与其它后台样式撞名）。
- **输出转义**：后台页 `$zbp->host` 增加 `htmlspecialchars`；数值输出统一显式 `(int)` 转换。
- **DNS 预取说明**：后台按官方规范明确写出「仅做 DNS 预解析，不建立连接、不预取资源，
  本插件不会向这些域名发送任何 API 请求」，并给出应填写何种域名的示例。
- **README 补齐**：新增「使用与设置」「隐私」「第三方组件」「许可证」章节。

### 未改动

- Hook 数量仍为 2 个（`Filter_Plugin_Zbp_MakeTemplatetags`、`Filter_Plugin_Admin_LeftMenu`）。
- 默认黑名单仍为 7 条通用关键词；迁移仍不触碰用户已保存的值。
- 不建表、不写文件、不联网、无遥测。

### 验证

48 项实测全部通过（安装 / 启用 / 停用 / 重装 / 升级 / 保存 / 权限 / XSS / CSRF /
非法与超长输入 / 前后台各页面 / 清除配置 / 性能 / 日志），详见 `docs/TEST_MATRIX.md`。
 · at8_pagespeed
