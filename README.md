...
- 本插件的三个 JS 均使用 ES6 语法（`let` / `const` / 箭头函数，按官方《注意事项速查表》建议放弃 `var`）：Chromium 49+ / Firefox 44+ / Safari 10+。在 IE 等不支持 ES6 的引擎中脚本不会执行，其错误被隔离在各自脚本内，**页面其它功能不受影响**（预加载与懒加载退化为不生效，而非报错）。
...
| 升级 | `at8_pagespeed_migrate_config()` 按 `ConfigVer` 逐级迁移配置，不重建用户数据；由 `InstallPlugin_` / `UpdatePlugin_` / 设置页加载三处调用，覆盖手动覆盖文件升级的场景 |
...
- **JS 注入安全加固**：黑名单关键字通过 `json_encode` 内联到 `<script>` 时补 `JSON_HEX_TAG` / `JSON_HEX_AMP` / `JSON_HEX_APOS` / `JSON_HEX_QUOT` 四个 HEX 标志，防管理员配置中含 `</script>` / `<!--` / `"` 等字符时 breakout `<script>` 上下文。HTTP 响应（`Content-Type: application/json`）不受影响；
...
- 同步更新 `include.php` 版本常量、`plugin.xml` 元数据、`CHANGELOG.md` 与发布检查清单。

### 1.0.5（2026-09-23）

- **最低 PHP 版本提高至 7.4**（`plugin.xml` 的 `<phpver>`，即应用中心的安装门槛）：
  低于 7.4 的环境会在安装阶段被拦下。依据——全量文件在 PHP 7.3.4 通过 `php -l`（严于 7.4），
  运行时在 PHP 8.2 实测零报错，且源码未使用任何 PHP 8.0+ 专有语法
  （无 `?->` / `match` 表达式 / `str_contains` / `#[Attribute]` / 构造器提升 / 联合类型 / `enum` / `readonly`）；
- **DNS 预取域名改为保存时即归一化**：原先只在输出时校验、库里存原始输入；现保存与输出共用同一套规则，非法项（含空格、带路径的 URL、脚本片段）直接丢弃，带协议的输入归一化为纯域名；
- **JS 放弃 `var`**：两个自研脚本改用 `let` / `const`（对齐官方《注意事项速查表》），功能不变；
- 同步更新 `include.php` 头部注释、README「兼容性」「浏览器」与发布检查清单。
...
- **元数据**：补充 `<adapted>` 的取值依据（与系统 `$zbp->version` 做数值比较，`172900` 面向 1.7 系列、可拦住 1.6 及更早版本）。
